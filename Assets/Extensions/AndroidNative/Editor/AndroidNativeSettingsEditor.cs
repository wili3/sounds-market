...
public class AndroidNativeSettingsEditor : Editor {
	...
	private void UpdataAndroidNativeAARs() {
		...
		if (AndroidNativeSettings.Instance.GoogleMobileAdAPI)
			PluginsInstalationUtil.EnableGoogleAdMobAPI ();
		else
			PluginsInstalationUtil.DisableGoogleAdMobAPI ();

		//Monipulate play-services-games.aar
		if (AndroidNativeSettings.Instance.GooglePlayServicesAPI)
			PluginsInstalationUtil.EnableGooglePlayServicesAPI ();
		else
			PluginsInstalationUtil.DisableGooglePlayServicesAPI ();

		//Manipulate play-services-gcm.aar
		if (AndroidNativeSettings.Instance.PushNotificationsAPI)
			PluginsInstalationUtil.EnablePushNotificationsAPI ();
		else
			PluginsInstalationUtil.DisablePushNotificationsAPI ();

		//Manipulate play-services-plus.aar
		if (AndroidNativeSettings.Instance.EnablePlusAPI)
			PluginsInstalationUtil.EnableGooglePlusAPI ();
		else
			PluginsInstalationUtil.DisableGooglePlusAPI ();

		//Manipulate play-services-appinvite.aar
		if (AndroidNativeSettings.Instance.EnableAppInviteAPI)
			PluginsInstalationUtil.EnableAppInvitesAPI ();
		else
			PluginsInstalationUtil.DisableAppInvitesAPI ();

		//Manipulate play-services-analytics.aar
		if (AndroidNativeSettings.Instance.AnalyticsAPI)
			PluginsInstalationUtil.EnableAnalyticsAPI ();
		else
			PluginsInstalationUtil.DisableAnalyticsAPI ();

		//Manipulate play-services-auth.aar
		if (AndroidNativeSettings.Instance.GoogleOAuthAPI)
			PluginsInstalationUtil.EnableOAuthAPI ();
		else
			PluginsInstalationUtil.DisableOAuthAPI ();

		//Manipulate play-services-drive.aar
		if (AndroidNativeSettings.Instance.GoogleCloudSaveAPI)
			PluginsInstalationUtil.EnableDriveAPI ();
		else
			PluginsInstalationUtil.DisableDriveAPI ();
	}
	...
	public static void UpdateManifest() {
		...
		AN_PropertyTemplate targetSdk = Manifest.GetOrCreatePropertyWithTag ("uses-sdk");
		targetSdk.SetValue ("android:targetSdkVersion", "23");
